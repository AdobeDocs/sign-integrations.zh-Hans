...
要配置 [!DNL Veeva Vault] 要与Adobe Sign集成，您需要实施以下步骤：

**步骤 1.** 创建一个名为“Adobe Sign Admin Group”的新组。

**步骤 2.** [部署包](https://helpx.adobe.com/content/dam/help/en/PKG-AdobeSign-Integration.zip)的
...
#### 应用程序角色 {#create-application-roles}

必须创建名为的应用程序角色 *Adobe Sign管理员角色*&#x200B;的 必须在符合“Adobe签名”条件的每个文档类型的生命周期中定义此。 对于每个Adobe Sign特定生命周期状态，都会添加具有相应权限的Adobe Sign管理员角色并进行配置。

![创建应用程序角色的图像](images/create-application-roles.png)

### 3.设置安全性配置文件 {#security-profiles}

为了成功集成Vault，一个名为 *Adobe Sign集成配置文件* 已创建且其权限已设置为 *Adobe Sign Admin Actions*&#x200B;的 Adobe Sign集成配置文件被分配给系统帐户，并由集成在调用Vault API时使用。 此配置文件允许具有以下权限：
...
对于在保管库中需要访问Adobe Sign历史记录的用户，其安全配置文件必须对签名、签名者和签名事件对象具有读取权限。
...
要确保系统帐户用户属于特定文档生命周期的Adobe Sign管理员组，您必须创建用户角色设置记录。 具体操作如下：
...
### 5.创建文档类型组 {#create-document-type-group}
...
**注意：** 如果用户角色设置对象不包含引用文档类型组对象的字段，则必须添加该字段。

### 6.创建用户角色设置 {#create-user-role-setup}

正确配置生命周期后，系统应确保DAC为适用于Adobe Sign进程的所有文档添加Adobe Sign管理员用户。 为此，请创建相应的用户角色设置记录，其中指定：

* 文档类型组为“Adobe Sign文档”、
* 作为“Adobe Sign管理员角色”的应用程序角色，以及
* 集成用户。

![用户角色设置的图像](images/user-role-setup.png)

**注意：** 如果用户角色设置对象不包含引用文档类型组对象的字段，则必须添加该字段。 要执行此操作，请转到对象>用户角色设置>字段，并完成所需步骤，如下图所示。

![用户角色设置的图像](images/create-setup-field.png)

### 7.设置文档字段 {#create-fields}

要与Adobe Sign建立集成，需要以下两个新的共享文档字段：
...
1. 转到“配置”选项卡并选择 **文档字段** > **共享字段**&#x200B;的
1. 在显示区段字段中，选择创建显示区段并分配Adobe签名作为区段标签。

   ![文档详细信息的图像](images/create-display-section.png)

1. 对于两个共享文档字段(signature__c和allow_adobe_sign_user_actions__c)，请使用“Adobe签名”部分标签更新UI部分。
1. 将这三个共享字段添加到所有符合文档签名条件的Adobe类型。 为此，在“基本文档”页面中，选择 **添加** > **现有共享字段** 右上角。

   ![文档详细信息的图像](images/add-existing-fields.png)

   ![文档详细信息的图像](images/use-shared-fields.png)

1. 请注意，这两个字段必须具有仅允许Adobe Sign管理员组成员更新其值的特定安全性。

   ![文档详细信息的图像](images/security-overrides.png)

1. 管理员必须添加现有的共享字段 *禁用电子仓库叠加(disable_vault_overlays__v)* 对于所有有资格使用“Adobe签名”的文档类型，请将其设置为“活动”。 或者，该字段可以具有特定的安全性，该安全性仅允许Adobe Sign管理员组的成员更新其值。

   ![允许Adobe Sign用户操作的图像](images/allow-adobe-sign-user-actions.png)

### 8.声明文档演绎版 {#declare-renditions}

Vault集成使用名为*Adobe Sign Rendition(adobe_sign_rendition__c)的新格式副本类型将已签名的PDF文档上传到Adobe Sign。 应针对有资格使用“Adobe签名”的每个文档类型声明Adobe Sign呈现形式。

![呈现类型的图像](images/rendition-type.png)

![呈现类型的图像](images/edit-details-clinical-type.png)

### 9.更新Web操作 {#web-actions}
...
### 10.更新文档生命周期 {#document-lifecycle}

对于有资格进行Adobe签名的每种文档类型，必须通过添加新生命周期角色和状态来更新相应的文档生命周期。
...
1. 添加生命周期角色
   ...
   必须将Adobe Sign管理员应用程序角色添加到有资格进行Adobe签名的文档使用的所有生命周期中，如下所示。
...
2. 创建生命周期状态。 为此，请转到 **设置** > **配置** > **文档生命周期** > **一般生命周期** > **状态** > **创建**&#x200B;的 接下来，创建以下状态：
...
此状态不需要用户操作。 此状态必须具有允许Adobe Sign管理员角色执行以下操作的安全性：查看文档、查看内容和编辑字段。

下图说明了Adobe Sign协议与Vault文档状态之间的映射，其中“Adobe签名前”状态为“草稿”。

![Adobe Sign Vault映射的图像](images/sign-vault-mappings.png)

### 11.将Adobe Sign阶段添加到生命周期阶段组中的常规生命周期

![Adobe Sign Vault映射的图像](images/add-adobe-sign-stage.png)

### 12.设置用户角色在生命周期状态下的权限

您必须在“生命周期状态”为每个用户角色设置相应的权限，如下图所示。

![Adobe Sign Vault映射的图像](images/set-user-role-permissions.png)

### 13.根据文档状态和用户角色设置原子安全

![Adobe Sign Vault映射的图像](images/set-atomic-security.png)

### 14.创建文档消息以取消Adobe Sign

![Adobe Sign Vault映射的图像](images/create-cancel-message.png)
...
1. 在打开的Adobe Sign登录页面中，提供帐户管理员电子邮件地址和密码，然后选择 **[!UICONTROL 歌唱]**&#x200B;的
...
1. 选择 **[!UICONTROL 设置]** 选项卡。
   ...
   “设置”页面显示可用连接，在首次设置连接时不显示任何连接，如下所示。
...
1. 在打开的“添加连接”对话框中，提供所需的详细信息，包括 [!DNL Veeva Vault] 凭据。
   ...
   Adobe Sign凭据通过初始Adobe Sign登录自动填充。
