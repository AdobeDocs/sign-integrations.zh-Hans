...
本文档旨在帮助 [!DNL Workday] 管理员了解如何自定义 [!DNL Workday] 业务流程，包括Adobe Sign以获取电子签名。 在中使用Adobe Sign [!DNL Workday]，您必须知道如何创建和修改 [!DNL Workday] 例如：
...
[!UICONTROL Adobe Sign电子签名功能] 呈现为 [!UICONTROL 审阅文档步骤] 操作 [!UICONTROL 业务流程框架(BPF)] 和“分发文档”任务。

## [!UICONTROL 审阅文档步骤] {#review-document-step}

Adobe Sign [!DNL Workday] 通过 [!UICONTROL 审阅文档步骤] 可以添加到 [!DNL Workday]，包括 [!UICONTROL 优惠], [!UICONTROL 分发文档和任务], [!UICONTROL 建议报酬]等。

您可以参考 [[!DNL Workday] 社区文章 [!UICONTROL 审阅文档步骤]](https://doc.workday.com/#/reader/3DMnG~27o049IYFWETFtTQ/TboWWKQemecNipWgxLAjqg).

两者之间有1:1的关系 [!UICONTROL [!UICONTROL 审阅文档步骤]s] 和与Adobe Sign的可计费交易。 您可以将多个文档组合在一个文档中 [!UICONTROL 审阅文档步骤] 并且它们以单个包的形式呈现以供签名。

**注意**:仅单个 *动态* 文档可以在特定的 [!UICONTROL 审阅文档步骤].

定义函数 [!UICONTROL 审阅文档步骤]:

1. 插入 [!UICONTROL 审阅文档步骤].
1. 指定可以对 [!UICONTROL 审阅文档步骤].
...
1. 在签名网格中添加行.
   ...
   * 当所有行都已签名后， [!UICONTROL 审阅文档步骤] 完成。

1. 指定要签名的文档。如果文档是 [!UICONTROL 聘用业务流程]，您可以从“生成文档”步骤中使用它。 否则，请选择一个现有的文档或报告。

1. 您可以根据需要对任意数量的文档重复步骤 3。

   ![配置审阅文档步骤](images/configure-rd-stepsmaller-575.png)

1. （可选）添加“重定向用户”以捕获“拒绝签名”操作。 当用户拒绝时， [!DNL Workday] 将文档重新路由到已配置的安全组以供审阅。

从 [!UICONTROL 审阅文档步骤]，选择 **[!UICONTROL 业务流程]** > **[!UICONTROL 维护重定向]**. 接下来，选择以下选项之一：
...
选件业务流程是 [!UICONTROL 作业应用程序动态业务流程] 必须配置为执行选件业务流程。 将作业应用程序状态移至“[!UICONTROL 优惠]&quot;或&quot;[!UICONTROL 提供优惠]”。

在下例中， [!UICONTROL 审阅文档步骤] 正在为北美和日本使用动态文档步骤。
...
下面的动态聘用信是在 [!DNL Workday] 富文本编辑器。 中突出显示的项目 *灰色* 是 [!DNL Workday] 提供了引用上下文数据的对象。

{{括号}} 中的项目是 [Adobe 文本标记](https://adobe.com/go/adobesign_text_tag_guide_cn)。

![动态表单示例](images/script.png)

在 [!UICONTROL 审阅文档步骤]，动态文档从上一步引用，并通过两个签名组定义顺序签名过程。
...
在 [!DNL Workday] 30，“成批分发文档”或“任务”任务可用于将单个文档发送给由单个签名者组成的大型组(&lt;20K)。 它仅限于每个文档的单个签名。通过访问“[!UICONTROL 创建分发文档或任务]”操作。

示例：向所有具有 [!UICONTROL 全球现代服务]. 如果需要，您可以进一步将其过滤给单个管理器。

您还可以访问 **查看分发文档或任务** 报告以跟踪分发进度。
...
在 [!DNL Workday] “签名周期”禁止Adobe Sign发送的所有电子邮件通知。 将通知用户其中的挂起操作 [!DNL Workday] 收件箱。

当文档由所有签名组签名后，已签名文档的副本将通过电子邮件分发给签名组的所有成员。

要抑制此行为，您可以联系 [!UICONTROL Adobe Sign成功经理] 或 [Adobe Sign支持团队](https://adobe.com/go/adobesign-support-center).

在 [!DNL Workday]，您可以在完整的流程记录中访问已签名的文档。 您可能会发现：
...
![示例 [!DNL Workday] 聘函](images/offer.png)
...
在 [!DNL Workday] community有几篇关于如何排除集成故障和生成文档的好文章：
...
Adobe Sign客户应联系其客户成功经理以获得支持。 或者， [!UICONTROL Adobe技术支持] 可以通过电话联系：1-866-318-4100，等待产品列表，然后输入：4和2（根据提示）。
